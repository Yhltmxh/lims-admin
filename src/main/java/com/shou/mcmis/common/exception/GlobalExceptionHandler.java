...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Result<?>> handleBindException(org.springframework.validation.BindException e) {
        String msg = e.getBindingResult().getFieldErrors().stream()
                .map(f -> f.getField() + ": " + f.getDefaultMessage())
                .collect(Collectors.joining("; "));
        log.warn("参数绑定失败: {}", msg);
    ...
    public ResponseEntity<Result<?>> handleDuplicateKey(DuplicateKeyException e) {
        ...
        return error(409, "数据已存在，请检查唯一字段");
    }
    ...
    public ResponseEntity<Result<?>> handleDataIntegrityViolation(DataIntegrityViolationException e) {
        ...
        return error(400, "数据操作违反完整性约束，请检查关联数据是否存在");
    }
    ...
    private ResponseEntity<Result<?>> error(int code, String message) {
